...
import com.revature.dto.ChangeReimAmountDTO;
...
public class ReimTicketController implements Controller {
	...
	private Handler changeReimAmount = (ctx) -> {
		User currentlyLoggedInUser = (User) ctx.req.getSession().getAttribute("currentuser");
		...
		ChangeReimAmountDTO dto = ctx.bodyAsClass(ChangeReimAmountDTO.class); // Taking the request body -> putting the
																				// data into a new object

		ReimTicket changedReimTicket = this.reimticketService.changeReimAmount(currentlyLoggedInUser, reimInfo, reimticketId, 
				dto.getReimAmount());
	...
	private Handler addReimTicket = (ctx) -> {
		...
		ReimTicket addedReimTicket = this.reimticketService.addReimTicket(currentlyLoggedInUser, mimeType,
				reimticketType, reiminfo, content);
	...
	public void mapEndpoints(Javalin app) {
		...
		app.patch("/reimtickets/{reimticket_id}/reimamount", changeReimAmount);
