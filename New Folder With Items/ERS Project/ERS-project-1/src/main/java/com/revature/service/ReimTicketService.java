...
public class ReimTicketService {
	...
	public ReimTicket changeReimAmount(User currentlyLoggedInUser, String reimInfo, String reimticketId, int reimamount)
		...
		try {
			...
			// 1
			if (reimticket.getResolverId() == 0) { // if it's 0, it means there's no reimbursement amount for the
													// reimticket yet
				this.reimticketDao.changeReimAmount(reimticketId, reimamount, reimInfo, currentlyLoggedInUser.getId());
				...
				throw new PreviousTicketCompletedException(
						" Previous ReimTicket was completed. Modification of ReimTicket amount is unauthorized.");
	...
	public ReimTicket addReimTicket(User currentlyLoggedInUser, String mimeType, String reimticketType, String reimInfo,
		...
		int reimauthorId = currentlyLoggedInUser.getId(); // whoever is logged in, will be the actual author of the
															// reimticket

		ReimTicket addedReimTicket = this.reimticketDao.addReimTicket(reimticketType, reimauthorId,reimInfo ,content);
